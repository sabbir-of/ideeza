...
function getMessage(type) {
  ...
  if (fs.existsSync(path.join(process.cwd(), "./result.json"))) {
    let defaultrawdata = fs.readFileSync(
      ...
      if (pwconfig.reporter != undefined) {
        ...
        for (let i = 0; i < reporterarray.length; i++) {
          ...
          if (reportname == "allure-playwright") {
            HTMLmessage2 =
              "<html>" +
              "<style>table{position: absolute;z-index: 2;left: 50%;top: 50%;transform: translate(-50%,-50%);width: 100%;" +
              ...
              "th{background-color: #271980;font-size:12;color: #f0f0f2;font-family: 'Open Sans',Sans-serif;font-weight: 200;text-transform: uppercase;}" +
              "tr{width: 100%;background-color: #bedafa;font-size:12;font-family: 'Montserrat', sans-serif;}" +
              "tr:nth-child(even){background-color: #eeeeee;}" +
              "</style>" +
              "<body><p>Hi Team, <br><br>Test Execution for the project " +
              testConfig.Project +
              " is conducted in " +
              testConfig.Env +
              " Environment and its succesfully completed.<br>PFB the link and data table for the Test Execution Status.<br></p>" +
              "<p> Link : " +
              testConfig.Githubreportlink +
              "<br>" +
            ...
            HTMLmessage2 =
              ...
              Totalcount +
              "</td><td ALIGN=CENTER>" +
              defaultobject.Passedcount +
              "</td><td ALIGN=CENTER>" +
              defaultobject.Failedcount +
              "</td><td ALIGN=CENTER>" +
              Passpercentage +
              "</td><td ALIGN=CENTER>" +
              Failpercentage +
    ...
    if ((type = "TEXT")) {
      const Textmessage =
        ...
        "\n [Click Here](https://azmansabbir.github.io/LightShow-Automation/) " +
        "\n" +
